...
const getFunc = async (req, res, next) => {
   try {
       let result = [];
        ...
        res.status(404).json({
            message: 'Contact: Get fail, Id not match',
            error
        });
   }
}
   ...
   try { 
        if(req.params.id) {
            ...
            if( idRegex.test(id) ) {
                ...
                let result = await Contact.updateOne({_id: id}, {$set: updateQuery})
                res.status(201).json({
                    result
                })
            }
...
const deleteFunc = async (req, res, next) => {
    try {
        ...
        let idRegex = /^[0-9a-zA-Z]{24}$/
        if(idRegex.test(id)) {
            result = await Contact.deleteOne({_id: id});   
            res.status(201).json({
                message: 'Deleted !',
                result
            })
